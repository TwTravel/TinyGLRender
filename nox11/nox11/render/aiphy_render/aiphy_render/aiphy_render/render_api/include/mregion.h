...
//-----------------区域的一些特征-------------------------------------
	...
	ObjRegion(int level = 256, int pixel = 0);
	...
	ObjRegion * parent_;      // 区域的父节点指针
	ObjRegion * child_ ;	   // 区域的孩子指针
	ObjRegion * next_  ;	   // 区域的兄弟指针
	
	void accumulate(int x, int y);   //当前level增加像素
	void merge(ObjRegion * child);
	void getvariation(int delta);
	
	friend class MSER;
    void  update_childnum();
	void  SaveMserRegion(vector<ObjRegion>&ReginVec, double minVariation);
	void  PrintTree(FILE*file,int start);
	int GRegionMinSize, GRegionMaxSize; //区域面积
	int PicWidth, PicHeight; //区域面积
	void SetRegionAreaConstrain(int GRegionMinSize_,int GRegionMaxSize_,int PicWidth_,int PicHeight_)
	{
		 GRegionMinSize = GRegionMinSize_;
		 GRegionMaxSize = GRegionMaxSize_;
		 PicWidth       = PicWidth_ ;
		 PicHeight      = PicHeight_;
	};
 
	void SaveToFile(vector<ObjRegion>&RgVec, char*filename);
	void LoadFromFile(vector<ObjRegion>&RgVec, char*filename);
...
/// \param r   所判断的矩形 
/// \return    如果在返回1, 否则返回0
int RegionInRect(ObjRegion & p, ImgRECT &r)
{
	return p.x > r.left && p.x < r.right &&
		p.y > r.top  && p.y < r.bottom ;
};
...
/// \param  start  区域的层次信息
/// \return 无
void ObjRegion::PrintTree(FILE*file,int start)
{
	string blankstr;
	blankstr = ""; 
	...
	for(ObjRegion * child = child_; child; child = child->next_)
...
/// \param  T  待分析的某个区域 
/// \return 无 
vector<RPoint>* GetSubNode(ObjRegion*T)
{	
	int i;
	for(ObjRegion * child = T->child_; child; child = child->next_) 
	{
...
/// \return 无 
void ObjRegion::SaveMserRegion(vector<ObjRegion>&ReginVec,double minVariation)
{
	int i;
...
ObjRegion::ObjRegion(int level, int pixel) : level_(level), pixel_(pixel), area_(0),
...
inline void ObjRegion::accumulate(int x, int y)
{
	++area_;
	moments_[0] += x;
	moments_[1] += y;
...
/// \return   
void ObjRegion::merge(ObjRegion * subNode)  //合并区域
{  
...
/// \return   
void ObjRegion::update_childnum()
{
	childNum = 0;
	for (ObjRegion * child = child_; child; child = child->next_) {
...
/// \return   
void ObjRegion::LoadFromFile(vector<ObjRegion>&RgVec, char*filename)
{
	int i,j;
...
/// \param filename  保存的文件名
/// \return    
void ObjRegion::SaveToFile(vector<ObjRegion>&RgVec, char*filename)
{
	int i,j;
	int * DumpImg =  new int[rwidth * rheight];
...
/// \param filename  保存的文件名
/// \return  
void PrintMSERRegionAxis(vector<ObjRegion>&RgVec, char*filename)
{
  int i;
...
void DisPlayRegionCircle(vector<ObjRegion>&RgVec, int ImgWidth,int ImgHeight, char*filename)
{
	int i;
	C24BitMap CPic;
